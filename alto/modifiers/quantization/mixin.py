...
class QuantizationMixin(HooksMixin):
    ...
    _observers: Dict[str, Observer] = PrivateAttr(default_factory=dict)
    ...
    def end_calibration(self, model: torch.nn.Module):
        ...
        for obs_name, observer in self._observers.items():
            if observer.base_name == "weight":
                continue
    ...
    def _initialize_observers(self, module: torch.nn.Module, module_name: str):
        ...
        if input:
            if not is_attention:
                observer = initialize_observer(module, base_name="input")
                if observer is not None:
                    self._observers[f"{module_name}.input_observer"] = observer
            else:
                if hasattr(module, IMPL_ATTR):
                    observer = initialize_observer(module, base_name="q")
                    if observer is not None:
                        self._observers[f"{module_name}.q_observer"] = observer
                if hasattr(module, KV_CACHE_ATTR):
                    observer = initialize_observer(module, base_name="k")
                    if observer is not None:
                        self._observers[f"{module_name}.k_observer"] = observer
                    observer = initialize_observer(module, base_name="v")
                    if observer is not None:
                        self._observers[f"{module_name}.v_observer"] = observer

        # weight observers (used by `update_weight_zp_scale` or child modifier)
        if weight:
            observer = initialize_observer(module, base_name="weight")
            if observer is not None:
                self._observers[f"{module_name}.weight_observer"] = observer

        # output activations
        if output:
            observer = initialize_observer(module, base_name="output")
            if observer is not None:
