...
class AlpsModifier(SparsityModifierBase):
    ...
    def compress_modules(self):
        ...
        for module, observer in self._layer_observers.items():
            ...
            assert isinstance(observer, HessianObserver), "AlpsModifier requires hessian observer"
        ...
        for i_admm in range(self.alps_iterations):
            ...
            if (i_admm + 1) % self.alps_update_iter == 0:
                ...
                if prune_n == 0:
                    ...
                    Btest = Btest.scatter(src=torch.zeros((new_dim, prune_m - prune_n)).to(H.device),
                                          dim=1,
                                          index=loss_idx)
