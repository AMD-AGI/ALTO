...
@triton.jit
def blockwise_mxfp8_gemm_dequant_debug_kernel(
    a_ptr,
    b_ptr,
    c_ptr,
    a_s_ptr,
    ...
    """Debug kernel that dequantizes inside Triton and uses tl.dot."""
    tl.assume(BLOCK_SIZE_K % QUANT_BLOCK_SIZE == 0)
    ...
    if DEBUG_PRINT:
        tl.device_print("=== GEMM DEQUANT DEBUG ===")
    ...
    for i in range(num_blocks_k):
        ...
        # tl.dot expects rhs as [K, N], but _dequantize_fp8 expands scales along the
        # last dimension. Dequantize B in [N, K] form first, then transpose back.
        ...
        accumulator += tl.dot(a_dq, b_dq, out_dtype=tl.float32)
...
def blockwise_mxfp8_gemm_dequant_debug(
    a: torch.Tensor,
    a_s: torch.Tensor,
    b: torch.Tensor,
    b_s: torch.Tensor,
    ...
    """
    Debug GEMM path that dequantizes inside Triton and uses tl.dot.

    This is useful for isolating whether precision issues come from tl.dot_scaled
    itself or from later K-block accumulation behavior.
    """
    assert a.dim() == 2 and b.dim() == 2
    if a.dtype != b.dtype:
        raise ValueError(f"A/B FP8 dtypes must match, got a.dtype={a.dtype}, b.dtype={b.dtype}")
    ...
    # Keep the launch geometry heuristic simple and consistent across M/N/K so
    # the debug path does not introduce extra shape-dependent behavior outside
    # of the dequantize + tl.dot implementation itself.
    BLOCK_SIZE_M = 64 if M >= 64 else M
    BLOCK_SIZE_N = 64 if N >= 64 else N
    ...
    blockwise_mxfp8_gemm_dequant_debug_kernel[grid](
        a, b, c, a_s, b_s,
        stride_am, stride_ak,
        stride_bn, stride_bk,
        stride_cm, stride_cn,
...
class MXFP8LinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        x_lp, x_scales = torch.ops.modeloptimizer.convert_to_mxfp8(
            x_2d,
        ...
        w_lp, w_scales = torch.ops.modeloptimizer.convert_to_mxfp8(
            weight,
            block_size=BLOCK_SIZE_DEFAULT,
            mxfp_format=mxfp_format,
            axis=-1,
        ...
        x_dq = torch.ops.modeloptimizer.convert_from_mxfp8(
            x_lp,
            x_scales,
        ...
        w_dq = torch.ops.modeloptimizer.convert_from_mxfp8(
            w_lp,
            w_scales,
            output_dtype=output_dtype,
            block_size=BLOCK_SIZE_DEFAULT,
        ...
        if is_cdna4():
            y = torch.ops.modeloptimizer.blockwise_mxfp8_gemm(
                x_lp,
                x_scales,
                w_lp,
                w_scales,
