...
from modeloptimizer.config import register_sparsification_methods
    ...
    def optimize_subset(
        ...
        for layer_name, layer in layers_dict.items():
            ...
            if self.N > 0:
                ...
                idx = W_metric_paded.topk(self.M - self.N, dim=2,
                                          largest=False).indices
                mask = torch.zeros((W_height, Groups, self.M),
                                   dtype=torch.bool,
                ...
                blocks = W_metric.reshape(W_height // self.block_height,
                                          self.block_height,
                                          W_width // self.block_width,
                                          self.block_width).permute(0, 2, 1, 3)
                score = blocks.mean(
                    (2, 3)
                ) if self.block_saliency_metric == 'absmean' else blocks.amax(
                    (2, 3))
                ...
                W_mask = prune_blocks.repeat_interleave(self.block_height,
                                                        0).repeat_interleave(
                                                            self.block_width, 1)
            else:
