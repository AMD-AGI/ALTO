...
where $\mathcal{Q}_{\mathrm{E2M1}}$ denotes rounding to the nearest representable E2M1 value and $r_{\max}$ is the largest magnitude representable in E2M1. Dequantization reconstructs $\tilde{\mathbf{x}}$ from $(\hat{\mathbf{x}}, s)$.

### 2.2 Block Geometry: Canonical 1D Blocks and Training-Oriented 2D Blocks

The OCP MX specification defines block scaling along contiguous 1D segments only: each scale is shared by 32 consecutive elements along a single axis. ALTO implements this canonical layout as **1D block quantization**, assigning one UE8M0 scale per 32 elements along the reduction (inner-product) axis of a GEMM operand.

For inference, quantizing each operand once along its contraction axis is sufficient. Training is more demanding. Consider a linear layer with weight $\mathbf{W} \in \mathbb{R}^{N \times K}$ and forward pass $\mathbf{Y} = \mathbf{X}\mathbf{W}^{\top}$. The forward GEMM contracts along $K$, so $\mathbf{W}$ is naturally quantized with scales aligned to that axis. The backward pass, however, requires $\partial \mathcal{L}/\partial \mathbf{W} = (\partial \mathcal{L}/\partial \mathbf{Y})^{\top}\mathbf{X}$, which accesses $\mathbf{W}$ through its transpose and therefore along the $N$ axis. Under 1D MX blocking, forward and backward thus demand quantization along different axes. A single 1D-quantized weight tensor cannot serve both passes faithfully: one must re-quantize $\mathbf{W}$ along the backward axis—introducing a second, inconsistent QDQ and accept that the weight seen by the forward and backward GEMMs differ. We refer to the resulting mismatch as **forward–backward weight bias**.

To address this, we adopt **2D block quantization** as proposed in [2509.25149]—an extension beyond the MX specification, not part of the OCP standard. A weight matrix is partitioned into $32 \times 32$ tiles, each sharing one scale. Because the tile grid is defined over both spatial dimensions of $\mathbf{W}$, the same quantized representation is valid whether the matrix is consumed in the forward orientation or transposed in the backward pass. A single quantization of $\mathbf{W}$ therefore serves both forward and backward, eliminating the weight bias that 1D blocking necessarily introduces during training. As a secondary benefit, 2D blocking reduces scale metadata: for $\mathbf{W} \in \mathbb{R}^{N \times K}$, 1D blocking along one axis requires $NK/32$ scales, whereas 2D blocking requires only $(N/32)(K/32)$—roughly $32\times$ fewer scale values and scale-computation steps.

Activations remain on the canonical 1D layout in our recipe (Section 4.1), as this is compatible with randomized Hadamard transforms on the backward activation-gradient path.

### 2.3 Simulated MXFP4 Training

ALTO follows the widely adopted **master-weight** paradigm: optimizer states and parameter updates are maintained in FP32 or BF16, while each matrix multiplication is preceded by a quantize–dequantize (QDQ) round trip through MXFP4. Gradients flow through the quantizer via STE unless augmented by one of the techniques described in Section 4.

On hardware without native MXFP4 GEMM support, ALTO performs **fake quantization**: operands are cast to MXFP4 and immediately dequantized back to BF16/FP32 before a standard high-precision GEMM. This faithfully reproduces the numerical error induced by FP4 arithmetic but does not realize the memory-bandwidth or Matrix-Core throughput benefits of true FP4 execution. Even on AMD CDNA4 (gfx950), where `tl.dot_scaled` enables native block-scaled FP4 GEMM, the current training path incurs per-operator QDQ overhead and auxiliary transforms (e.g., Hadamard rotations) that are not yet fused into a single kernel.

---

## 3. Problem Setting

### 3.1 Benchmark: MLPerf Small MoE

We evaluate on the **MLPerf Small MoE** training benchmark, which specifies continued pretraining of a sparse MoE LLM on a subset of the C4 dataset. Our target model is **GPT-OSS-20B**, an OpenAI-released MoE architecture with the following salient properties:
...
We report validation cross-entropy loss on the held-out C4 validation set, measured at regular intervals throughout training. The BF16 baseline (`gpt_oss_20b_pretrain`) uses identical data, hyperparameters, and parallelism, differing only in the absence of MXFP4 QDQ in linear and grouped-GEMM operations.

---

## 4. Method

We organize accuracy recovery into a composable stack. The default recipe (`lpt_recipe.yaml`) enables hybrid block quantization, RHT, and SR; weight de-oscillation is optional.

Sections 4.1–4.3 describe techniques adopted from [2509.25149] and integrated in ALTO; we do not claim them as novel. Section 4.4 implements weight de-oscillation as a simplified variant of the OsciReset algorithm in TetraJet-v2 [2510.27527]. Section 5 covers additional methods explored but not retained in the default recipe.

### 4.1 Hybrid 1D/2D Block Quantization

**Problem.** The MX specification's 1D block layout is axis-oriented: scales are tied to the contraction dimension of a single GEMM orientation. In training, the forward and backward passes contract along different axes of the weight matrix—forward along the input dimension $K$, backward (via the transposed weight) along the output dimension $N$. Quantizing $\mathbf{W}$ in 1D therefore forces a choice between (i) performing two separate quantizations along the two axes, which doubles QDQ overhead and still leaves the two quantized views inconsistent with a single master weight, or (ii) reusing one axis's quantization in both passes, which introduces **forward–backward weight bias** because the scale grid seen by the forward GEMM differs from that seen by the backward GEMM. Activations present a separate concern: outlier redistribution via RHT (Section 4.2) requires 1D segments and is incompatible with 2D activation blocking.

**Solution.** We adopt a **1D–2D hybrid** layout (denoted *1d2d*) that respects the MX spec for activations while extending it for weights:

| Operand | Block geometry | Rationale |
|---------|---------------|-----------|
| Activations $\mathbf{X}$ | 1D (canonical MX; $32$-element segments) | Compatible with RHT on backward activation gradients (Section 4.2) |
| Weights $\mathbf{W}$ | 2D ($32 \times 32$ tiles; beyond MX spec) | **Single quantization** valid for both forward ($\mathbf{X}\mathbf{W}^{\top}$) and backward ($(\partial \mathcal{L}/\partial \mathbf{Y})^{\top}\mathbf{X}$); eliminates forward–backward weight bias; lower scale metadata cost |

2D block weight quantization is proposed in [2509.25149] to align forward and backward operands and reduce scale overhead; we adopt it in a hybrid 1d2d layout adapted to MX-format training (32×32 tiles, 1D activations) rather than NVFP4's 16×16 blocks.

For MoE grouped GEMM, expert weights $\mathbf{W} \in \mathbb{R}^{E \times N \times K}$ receive 2D blocking along the final two dimensions. One QDQ of each expert weight tensor is reused across both the forward grouped GEMM and the transposed contraction in the weight-gradient kernel, with no axis-dependent re-quantization and no discrepancy between the weight operand presented to the forward and backward graphs.

### 4.2 Randomized Hadamard Transform
...
**Solution.** Following [2509.25149], we apply a **randomized Hadamard transform** (RHT) to activation gradients during the backward pass. Let $\mathbf{H} \in \mathbb{R}^{32 \times 32}$ be a Hadamard matrix constructed via Sylvester's recursion, further randomized by random sign flips and column permutations. For 1D-blocked activations, the gradient $\mathbf{G}$ with respect to the layer output is transformed as

$$
\mathbf{G}' = \mathbf{G}\,\mathbf{H}^{\top}
$$

before MXFP4 quantization. Because $\mathbf{H}$ is orthogonal (up to scaling), the transform redistributes energy across coordinates without altering the underlying linear operator's gradient in expectation over randomization. RHT is applied exclusively on the 1D activation path and is therefore mutually exclusive with 2D activation blocking—motivating the hybrid layout of Section 4.1.

### 4.3 Stochastic Rounding on Gradients
...
**Solution.** As in [2509.25149], we employ **stochastic rounding** (SR) during gradient quantization in the backward pass. Each element is rounded up or down with probability proportional to its fractional distance from the two adjacent representable values, yielding an unbiased estimator of the pre-quantization gradient in expectation. SR is enabled via `use_sr_grad` and is implemented with a pseudo-random number generator; on CDNA4, rounding is accelerated through inline assembly (`v_cvt_scalef32_sr_pk_fp4_*`). Forward-pass quantization of weights and activations remains deterministic.

**Trade-off.** SR injects additional quantization noise, reducing per-operator signal-to-noise ratio (SNR) relative to deterministic rounding (Section 6.1). In end-to-end training, the reduction in systematic bias dominates this local SNR penalty.

### 4.4 Weight De-Oscillation
...
**Solution.** We implement a simplified variant of **OsciReset** from TetraJet-v2 [2510.27527]. Over a window of $P$ optimizer steps (default $P{=}200$), we accumulate per-element $\ell_1$ travel distances:
...
### 5.1 Differential Gradient Estimation (DGE)
...
$$
f(x) = \left(\frac{\delta}{2}\right)^{1 - 1/k}\,\mathrm{sgn}(x - m)\,|x - m|^{1/k} + m,
$$

$$
f'(x) = \frac{1}{k}\left(\frac{\delta}{2}\right)^{1 - 1/k}\,|x - m|^{1/k - 1},
$$

where $f'(x)$ is clamped to $[0, 3]$ before application. The weight gradient is modulated as

$$
\nabla_{\mathbf{W}} \leftarrow \nabla_{\mathbf{W}} \odot f'(\mathbf{W}).
$$

For MXFP4, bin widths $\delta$ and midpoints $m$ are determined by the E2M1 representable grid (15 breakpoints for FP4). Operator-level tests show marginal SNR improvement on $\partial \mathcal{L}/\partial \mathbf{W}$, but end-to-end validation loss degrades severely ($+0.19$ at 16,128 steps with GBS=16; Table 5), confirming that the modified DGE formula does not help at GPT-OSS-20B scale.

### 5.2 Outlier Clipping
...
Both modes degrade operator-level SNR (static clipping lowers the SNR floor to approximately 6 dB) and do not improve end-to-end validation loss: static clipping is marginally helpful ($\Delta = -0.007$), while dynamic clipping causes a large regression ($+0.18$; Table 5).

### 5.3 Macro-Block Scaling

[2603.08713] proposes **Macro Block Scaling (MBS)** as a two-level scheme for MXFP4: a coarse macro-scale is applied before the standard 32-element MX block quantization, allocating higher-precision scaling at a larger granularity to better preserve outliers. ALTO implements this as `two_level_scaling: blockwise` in the LPT recipe.

Our configuration applies MBS with 128×128 blocks on weights and 1×128 blocks on activations (one macro-scale per row over 128 consecutive elements along the quantization axis). Within each macro-block, the scale is derived so that the block maximum maps to the largest E2M1 representable magnitude 6.0:

$$
s_{\mathrm{macro}} = \frac{6}{\max_{i \in \mathrm{block}} |x_i|}.
$$

The macro-scale is encoded as a shared mantissa: only the upper bits of the FP32 scale factor are stored (8-bit metadata per macro-block), and the value is reconstructed for pre-scaling before MXFP4 QDQ and descaling afterward. Weights use a 2D $128 \times 128$ macro grid aligned with 2D MX blocking; activations use a 1D $1 \times 128$ layout consistent with 1D MX segments and RHT.

MBS improves single-operator SNR relative to plain 1d2d quantization in our unit tests (Section 6.1), confirming that coarser high-precision scaling reduces local saturation error. At GPT-OSS-20B scale, however, MBS yields only a modest validation-loss improvement ($-0.011$ vs. the default recipe at 16,128 steps; Table 5) and is not enabled in the production recipe.

### 5.4 Low-Rank Outlier Compensation
...
### 6.1 Operator-Level Numerical Fidelity

Before end-to-end training, we verify that ALTO's MXFP4 Linear and grouped GEMM autograd paths reproduce a high-precision reference. Each configuration is evaluated by running a forward pass and a full backward pass through an MSE loss, then comparing the MXFP4 outputs and gradients against a **BF16 reference** that performs the same computation without MXFP4 quantization.

**Metric.** We report signal-to-noise ratio (SNR) in decibels:

$$
\mathrm{SNR} = 10 \log_{10} \frac{\sum_i x_i^2}{\sum_i (x_i - \hat{x}_i)^2},
$$

where $x$ is the BF16 reference tensor and $\hat{x}$ is the MXFP4 result. SNR is computed in FP32 accumulation for numerical stability. We report three quantities per configuration: forward output ($\mathcal{O}$), input gradient ($\partial/\partial \mathbf{X}$), and weight gradient ($\partial/\partial \mathbf{W}$).
...
i.e., each element independently receives an additional outlier perturbation with probability $0.5\%$, scaled to roughly **100×** the typical magnitude. Inputs, weights, and loss targets are all generated by this procedure.

**Test Configuration.** Table 1 reports MXFP4 Linear with BF16 dtype and 1d2d blocking (1D for activations, 2D for weights). Table 2 reports MoE grouped GEMM with the same 1d2d layout and BF16 dtype. All numbers are measured on the fake-quantized path (QDQ + high-precision GEMM) used in our current training stack.

**Table 1.** SNR (dB) of MXFP4 linear relative to BF16 reference.

| Configuration | Forward | $\partial/\partial \mathbf{X}$ | $\partial/\partial \mathbf{W}$ |
|---------------|---------|-------------------------------|-------------------------------|
...
| Method | Validation loss @ 13056-step | Validation loss @ 13824-step | Validation loss @ 15360-step | Validation loss @ 16128-step |
|--------|------------------------------|------------------------------|------------------------------|------------------------------|
...
| Method | Validation loss @ 5376-step | Validation loss @ 6144-step | Validation loss @ 6912-step | Validation loss @ 7680-step |
|--------|------------------------------|------------------------------|------------------------------|------------------------------|
...
Future work will extend training toward the full 1.2M-step MLPerf horizon, fuse quantization and transforms into a single kernel pipeline, and pursue audited MLPerf Training submission on native FP4 hardware.
...
4. [2501.17116] Wang et al. *Optimizing Large Language Model Training Using FP4 Quantization.* arXiv:2501.17116. — Differentiable gradient estimation (DGE); ALTO uses a modified, segment-continuous formula (Section 5.1).
