...
def _load_quantization_modifier(monkeypatch):
    ...
    math (both live in ``alto.modifiers.quantization.mx``); the ``format`` field
    injection lives in ``alto.modifiers.quantization.format_registry``. All
    ``sys.modules`` mutations go through ``monkeypatch`` so they are reverted at
    test teardown and cannot leak into other tests in the same process.
    """
...
def _load_mx6_modifier_from_recipe(monkeypatch):
...
def test_mx6_wa_recipe_toy_linear_lifecycle(monkeypatch):
    modifier, quantize_mod = _load_mx6_modifier_from_recipe(monkeypatch)
