...
def _midmax_scale_ref(amax: float, block_size: int = 32) -> int:
    ...
    Mirrors the triton kernel logic in _calculate_scales with SCALE_SELECTION="midmax".
    """
    import struct
...
def test_midmax_scale_matches_ref_random(dtype):
    """
    Triton kernel (blockscale_selection="midmax") scales must match the pure-Python reference
    on random data.
    """
    torch.manual_seed(42)
    x = torch.randn(128, 64, dtype=dtype, device="cuda")
    _, scales_triton = convert_to_mxfp4(x, blockscale_selection="midmax")
...
# ---------------------------------------------------------------------------
# 3. Kernel output: blockscale_selection="midmax" vs blockscale_selection="default" differ appropriately
# ---------------------------------------------------------------------------
...
def test_midmax_differs_from_round_even_on_outliers():
    ...
    _, scales_midmax = convert_to_mxfp4(x, blockscale_selection="midmax")
    _, scales_round_even = convert_to_mxfp4(x, blockscale_selection="default")
...
def test_midmax_false_matches_pytorch_ref():
    ...
    _, scales_triton = convert_to_mxfp4(x, blockscale_selection="default")
...
# ---------------------------------------------------------------------------
# 4. Quantize → dequantize roundtrip with blockscale_selection="midmax"
# ---------------------------------------------------------------------------
...
def test_midmax_roundtrip(tensor_shape, axis, is_2d_block, dtype):
    """
    Roundtrip (quant → dequant) with blockscale_selection="midmax" should reconstruct
    the input with reasonable accuracy (MAE within one E2M1 quantum of the
    block scale).
    """
    x = prepare_data(tensor_shape, dtype)
    data_lp, scales = convert_to_mxfp4(x, axis=axis, is_2d_block=is_2d_block, blockscale_selection="midmax")
...
def test_midmax_all_zeros():
    ...
    data_lp, scales = convert_to_mxfp4(x, blockscale_selection="midmax")
...
def test_midmax_negative_values():
    ...
    _, scales_pos = convert_to_mxfp4(x_pos, blockscale_selection="midmax")
    _, scales_neg = convert_to_mxfp4(x_neg, blockscale_selection="midmax")
...
def test_midmax_single_outlier_block():
    ...
    _, scales = convert_to_mxfp4(x, blockscale_selection="midmax")
...
# ---------------------------------------------------------------------------
# 7. Consistency: blockscale_selection="midmax" gives same result across dtypes (f32 vs bf16)
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("use_asm", [False])
def test_midmax_dtype_consistency(use_asm):
    """
    Quantizing the same tensor in float32 and bfloat16 with blockscale_selection="midmax"
    ...
    _, scales_f32 = convert_to_mxfp4(x_f32, blockscale_selection="midmax", use_asm=use_asm)
    _, scales_bf16 = convert_to_mxfp4(x_bf16, blockscale_selection="midmax", use_asm=use_asm)
...
def test_midmax_asm_matches_non_asm():
    """On CDNA4, use_asm=True with blockscale_selection="midmax" must match use_asm=False."""
    if not is_cdna4():
        pytest.skip("ASM path requires CDNA4 hardware")

    x = prepare_data((128, 64), torch.float32)
    data_lp_asm, scales_asm = convert_to_mxfp4(x, blockscale_selection="midmax", use_asm=True)
    data_lp_ref, scales_ref = convert_to_mxfp4(x, blockscale_selection="midmax", use_asm=False)
