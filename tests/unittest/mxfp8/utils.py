...
def mxfp8_attention_backward_reference_stage2(
    ...
    """Pure-PyTorch golden reference for the **Stage-2** MXFP8 (e4m3) backward.

    Where Stage-1 (``mxfp8_attention_backward_reference``) quantizes only Q/K/V
    (2D-block along head_dim) and runs every backward matmul in fp32, Stage-2
    additionally quantizes **each matmul operand along that dot's reduction
    axis** — a numerical model of what ``tl.dot_scaled`` will do in the kernel.
    A kernel-vs-this gap then isolates Triton port bugs from mxfp8 error, and a
    this-vs-bf16-SDPA gap measures the *full* mxfp8 backward quantization error
    (the signal that decides whether all-e4m3 backward is viable or must go
    e5m2), all **without CDNA4** (no ``tl.dot_scaled`` here; plain fp32 matmul
    on dequantized operands).

    Quantization axis per dot (see plan §9.5)::

        dP = dO @ Vᵀ            reduction head_dim_v : dO,V   2D-block axis=-1
        S  = Q  @ Kᵀ            reduction head_dim   : Q,K    2D-block axis=-1  (reused from fwd)
        dV = Pᵀ @ dO            reduction seqlen_q   : P,dO   1D-block along sq
        dQ = dS @ K             reduction seqlen_k   : dS,K   1D-block along sk
        dK = dSᵀ @ Q            reduction seqlen_q   : dS,Q   1D-block along sq

    2D/1D rule (mirrors the forward decisions): operands whose reduction axis is
    head_dim/head_dim_v use 2D-block (like fwd Q/K/V); operands whose reduction
    axis is a sequence length use 1D-block along that seqlen (like fwd P).

    Same top-left causal mask as the other references, so causal tests must keep
    ``seqlen_q == seqlen_k``.

    Args mirror ``mxfp8_attention_backward_reference``; ``o`` / ``softmax_lse``
    must be the forward-reference outputs. Returns ``(dq, dk, dv)``.
    """
    b, hq, sq, dqk = q.shape
    _, hk, sk, dvv = v.shape
    assert hq % hk == 0, f"nheads_q ({hq}) must be a multiple of nheads_k ({hk})"
    ...
    def qdq(x, axis, is_2d_block):
        return _mxfp8_qdq(x.to(torch.float32), axis=axis, is_2d_block=is_2d_block, block_size=block_size)

    # --- head_dim-reduction operands: 2D-block along head_dim (reuse fwd convention) ---
    q_hd = qdq(q, axis=-1, is_2d_block=True)  # for S (dots a/e)
    k_hd = qdq(k, axis=-1, is_2d_block=True)  # for S
    v_hd = qdq(v, axis=-1, is_2d_block=True)  # for dP (dots b/f)
    do_hd = qdq(do, axis=-1, is_2d_block=True)  # dO for dP, along head_dim_v

    # --- seqlen-reduction operands: 1D-block along the seqlen axis ---
    q_sq = qdq(q, axis=-2, is_2d_block=False)  # for dK (dot d)
    k_sk = qdq(k, axis=-2, is_2d_block=False)  # for dQ (dot g)
    do_sq = qdq(do, axis=-2, is_2d_block=False)  # dO for dV (dot c)

    if n_rep > 1:
        k_hd = k_hd.repeat_interleave(n_rep, dim=1)
        v_hd = v_hd.repeat_interleave(n_rep, dim=1)
        k_sk = k_sk.repeat_interleave(n_rep, dim=1)
    ...
    s = torch.matmul(q_hd, k_hd.transpose(-1, -2)) * sm_scale
    if causal:
        q_pos = torch.arange(sq, device=q.device)
        k_pos = torch.arange(sk, device=q.device)
    ...
    # dP = dO @ Vᵀ (reduction head_dim_v).
    dp = torch.matmul(do_hd, v_hd.transpose(-1, -2))
    delta = (o_f * do_f).sum(dim=-1)
    ds = p * (dp - delta[..., None])  # fp32 elementwise

    # dV = Pᵀ @ dO (reduction seqlen_q): P and dO 1D-block along sq.
    p_sq = qdq(p, axis=-2, is_2d_block=False)
    dv_full = torch.matmul(p_sq.transpose(-1, -2), do_sq)

    # dQ = sm_scale * dS @ K (reduction seqlen_k): dS along sk, K along sk.
    ds_sk = qdq(ds, axis=-1, is_2d_block=False)
    dq = sm_scale * torch.matmul(ds_sk, k_sk)

    # dK = sm_scale * dSᵀ @ Q (reduction seqlen_q): dS along sq, Q along sq.
    ds_sq = qdq(ds, axis=-2, is_2d_block=False)
    dk_full = sm_scale * torch.matmul(ds_sq.transpose(-1, -2), q_sq)
